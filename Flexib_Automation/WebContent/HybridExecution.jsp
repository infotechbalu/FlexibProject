...
<%
	...
	if(s2.equalsIgnoreCase("True")){ 
		   ...
		   // System.out.println(request.getRealPath("/"));
		    command=new String[]{"CMD","/C",request.getRealPath("/")+"\\ShoppingCart_Demo\\ShoppingCartDemo.bat"};
	...
	if(s2.equalsIgnoreCase("True1")){ 
	    String[] command;
	    String line=null;
	    
	   // System.out.println(request.getRealPath("/"));
	    command=new String[]{"CMD","/C",request.getRealPath("/")+"\\MavenwithCucumber\\MavenCucumber.bat"};
		ProcessBuilder buildProcess=new ProcessBuilder(command);
		buildProcess.directory(new File(System.getProperty("user.dir")));
		try {
			Process p=buildProcess.start();
